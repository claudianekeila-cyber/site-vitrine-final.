...
const App: React.FC = () => {
  ...
  const handleOpenAbout = () => {
    setShowAbout(true);
    // Pequeno delay para garantir que o componente foi renderizado antes de rolar
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-peach-50 selection:bg-orange-200 selection:text-terra-950">
      <Navbar onOpenAbout={handleOpenAbout} />
      <main>
        <Hero />
        <Services />
        <AuditTool />
        
        {/* Renderização Condicional: Só aparece se showAbout for true */}
        {showAbout && <About />}
